...
import { UserPlus, Plus, Phone, FileText, TrendingUp } from 'lucide-react'

export default function ClientsPage() {
  const { user } = useAuthStore()

  // Check permissions
  if (!['1', '2', '5'].includes(user?.role)) {
    return (
      <DashboardLayout>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight flex items-center">
              <UserPlus className="mr-3 h-8 w-8 text-indigo-600" />
              Clients
            </h1>
            <p className="text-muted-foreground">
        ...
        <Card className="border-dashed border-2">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              La gestion clients sera bientôt disponible
            </CardDescription>
          </CardHeader>
          <CardContent className="text-center space-y-4">
            <div className="grid gap-4 md:grid-cols-3 max-w-2xl mx-auto">
              <div className="p-4">
                ...
                <p className="text-sm text-muted-foreground">Coordonnées et communications</p>
              </div>
              <div className="p-4">
                <FileText className="h-12 w-12 mx-auto mb-2 text-muted-foreground" />
                <h3 className="font-medium">Contrats</h3>
                <p className="text-sm text-muted-foreground">Accords et conditions</p>
              </div>
              <div className="p-4">
                <TrendingUp className="h-12 w-12 mx-auto mb-2 text-muted-foreground" />
                <h3 className="font-medium">Historique</h3>
                <p className="text-sm text-muted-foreground">Suivi des prestations</p>
              </div>
            </div>
