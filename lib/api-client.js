...
export const authAPI = {
  // Login with OAuth2 form data (POST /api/v1/auth/login)
  ...
  // Test token validity (POST /api/v1/auth/test-token)
  testToken: () => api.post('/auth/test-token'),

  // Request password recovery (POST /api/v1/auth/password-recovery/{email})
  recoverPassword: (email) => api.post(`/auth/password-recovery/${email}`),

  // Reset password with token (POST /api/v1/auth/reset-password)
  resetPassword: (data) => api.post('/auth/reset-password', data),

  // Secure logout (POST /api/v1/auth/logout)
  logout: () => api.post('/auth/logout'),

  // Public user registration (POST /api/v1/auth/register)
...
export const usersAPI = {
  ...
  getUsers: (params = { include_deleted: true }) => api.get('/users/', { params }),
  ...
  // === Bulk Operations ===
  // Bulk user operations (POST /api/v1/users/bulk)
  bulkUserOperation: (data) => api.post('/users/bulk', data),
...
export const invitationsAPI = {
  ...
  // List invitations with status filtering (GET /api/v1/invitations/)
  getInvitations: (params = {}) => api.get('/invitations/', { params }),

  // Get invitation details by token - PUBLIC ENDPOINT (GET /api/v1/invitations/token/{token})
  getInvitationByToken: (token) => {
    // Create axios instance without auth interceptors for public endpoint
    const publicApi = axios.create({
      ...
      timeout: 30000,
      withCredentials: false, // No cookies needed for public endpoint
    })
    return publicApi.get(`/invitations/token/${token}`)
  },

  // Verify invitation token using auth endpoint - PUBLIC ENDPOINT (GET /api/v1/auth/test-token?token=xxx)
  verifyInvitationToken: (token) => {
    ...
    return publicApi.post(`/auth/test-token?token=${token}`)
  },
  ...
  acceptInvitation: (data) => {
    ...
    return publicApi.post('/invitations/accept', data)
  },
...
export const trialAPI = {
  // Get trial status (GET /api/v1/trial/status)
  getTrialStatus: (user) => api.get('/trial/status', { data: user }),

  // Get trial notifications (GET /api/v1/trial/notifications)
  getTrialNotifications: (user, unreadOnly = false) =>
    api.get('/trial/notifications', {
      params: { unread_only: unreadOnly },
      data: user
    }),

  // Mark notification as read (POST /api/v1/trial/notifications/{notification_id}/mark-read)
  markNotificationRead: (notificationId, user) =>
    api.post(`/trial/notifications/${notificationId}/mark-read`, user),

  // Get trial analytics (GET /api/v1/trial/analytics)
  getTrialAnalytics: (user) => api.get('/trial/analytics', { data: user }),

  // Extend grace period (POST /api/v1/trial/extend-grace)
  extendGracePeriod: (user) => api.post('/trial/extend-grace', user),

  // Get upgrade suggestions (GET /api/v1/trial/upgrade-suggestions)
  getUpgradeSuggestions: (user) => api.get('/trial/upgrade-suggestions', { data: user }),
}
...
export const utilsAPI = {
  // Health check simple (GET /api/v1/utils/health-check/)
  healthCheck: () => api.get('/utils/health-check/'),

  // Health check détaillé (GET /api/v1/utils/health)
  healthDetailed: () => api.get('/utils/health'),
  ...
  // System info (GET /api/v1/utils/system/info)
  getSystemInfo: () => api.get('/utils/system/info'),

  // Clear cache (POST /api/v1/utils/cache/clear)
  clearCache: () => api.post('/utils/cache/clear'),
}
