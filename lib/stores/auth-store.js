...
export const useAuthStore = create(
  persist(
    (set, get) => ({
      ...
      // 🚀 OPTIMIZED: Unified authentication method that replaces login + fetchUserData + checkAuth
      authenticateUser: async (credentials = null, skipCache = false) => {
        ...
        const authPromise = (async () => {
          try {
            ...
            // 2️⃣ Smart cache check (unless skipCache is true)
            ...
            // 3️⃣ Fetch fresh user and company data in one go
            ...
            if (USE_MOCK_DATA) {
              ...
              if (typeof window !== 'undefined') {
                ...
                sessionStorage.setItem('ferdi_user_cache', JSON.stringify({ user: userData, timestamp }))
                sessionStorage.setItem('ferdi_company_cache', JSON.stringify({ company: companyData, timestamp }))
              }
      ...
      fetchUserData: async () => {
        console.warn('⚠️ fetchUserData is deprecated, use authenticateUser() instead')
