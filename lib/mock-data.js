...
import { UserRole, UserStatus, UserInvitationStatus, CompanyStatus, SubscriptionPlan } from '@/lib/constants/enums'
...
export const MOCK_DATA = {
  ...
  // Sample users with different roles using new enum values
  users: [
    {
      id: 'user-admin-001',
    ...
    },
    {
      ...
      last_login_at: '2024-12-14T14:30:00Z',
    },
    {
      ...
      first_name: 'Thomas',
      last_name: 'Lefevre',
      full_name: 'Thomas Lefevre',
      mobile: '0667890123',
      role: UserRole.ACCOUNTANT,
      status: UserStatus.ACTIVE,
      is_active: true,
      created_at: '2024-04-01T09:00:00Z',
      last_login_at: '2024-12-15T11:15:00Z',
    },
  ],
  ...
  userRegistrationSuccess: {
    ...
    is_active: false, // Pending users are not active by default
...
export const mockHelpers = {
  // Current session storage for mock API
  currentSession: {
    user: null,
    token: null,
  },
...
export const mockAPI = {
  ...
  login: async (email, password) => {
    ...
    if (!loggedInUser) {
      return mockHelpers.errorResponse(MOCK_DATA.errors.userNotFound, 404)
    }

    // Check if user is active before allowing login
    if (!loggedInUser.is_active) {
      return mockHelpers.errorResponse({
        detail: 'Votre compte est en cours de validation par les super administrateurs. Veuillez attendre la validation pour utiliser FERDI.'
      }, 400)
    }

    // Store current session for mock API
    mockHelpers.currentSession = {
      user: loggedInUser,
      token: 'mock-jwt-token-12345'
    }
    
    console.log('🧪 Mock Login successful for:', loggedInUser.email, 'Role:', loggedInUser.role)
  ...
  getCurrentUser: async (token) => {
    ...
    // Return the user from current session, not always the first user
    const currentUser = mockHelpers.currentSession?.user
    if (currentUser) {
      console.log('🧪 Mock getCurrentUser returning:', currentUser.email, 'Role:', currentUser.role)
      return mockHelpers.successResponse(currentUser)
    }
    
    // Fallback to first admin user if no session
    console.warn('⚠️ No current session, returning default admin user')
  ...
  // Validate company code in real-time
  validateCompanyCode: async (code) => {
    await mockHelpers.delay(300)
    
    // Valid company codes for demo
    const validCodes = [
      'BRE-12345-ABC', 
      'PAR-67890-DEF', 
      'LYN-11111-GHI',
    ...
    if (validCodes.includes(code)) {
      const companyData = {
        code: code,
        name: code === 'BRE-12345-ABC' ? 'Transport Bretagne SARL' : `Entreprise ${code}`,
        city: code === 'BRE-12345-ABC' ? 'Quimper' : 'Ville',
      ...
      return mockHelpers.errorResponse({
        valid: false,
        message: 'Code entreprise introuvable ou inactif'
      }, 404)
    }
  },
}