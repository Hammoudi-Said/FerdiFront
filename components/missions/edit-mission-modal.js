...
export function EditMissionModal({ open, onOpenChange, mission, onSave }) {
  ...
  // Update form when mission changes
  useEffect(() => {
    if (mission) {
      const departureDate = new Date(mission.departure_date)
      const returnDate = mission.return_date ? new Date(mission.return_date) : null
      ...
      form.reset({
        ...
        return_time: returnDate ? returnDate.toTimeString().slice(0, 5) : '',
        passenger_count: mission.passenger_count || 1,
        ...
        estimated_cost: mission.estimated_cost || 0,
      })
    }
  }, [mission, form])
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Modifiez les détails de la mission {mission?.mission_number}
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-500">Numéro de mission</label>
              <p className="text-gray-900 bg-gray-50 px-3 py-2 rounded-md font-mono">{mission?.mission_number}</p>
              <p className="text-xs text-gray-500">Le numéro de mission ne peut pas être modifié</p>
            </div>
